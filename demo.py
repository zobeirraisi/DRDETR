...
def get_args_parser():
    ...
    parser.add_argument(
        "--img_path",
        # default="data/44015/img_11.jpg",
        default="demo_images/16.jpg",
        type=str,
        help="input image file for inference",
    )
    parser.add_argument(
        "--resume", default="out/checkpoint.pth", help="resume from checkpoint"
    )
    return parser
...
def draw_boxes(boxes, img, color="red"):
...
def main(args):
    ...
    t0 = time.time()
    source_img = Image.open(args.img_path).convert("RGB")

    corner_boxes = detect(source_img, model, transform)

    print(f"elapsed time = {time.time() - t0:.3f}s")

    draw_boxes(corner_boxes, source_img, color="cyan")
