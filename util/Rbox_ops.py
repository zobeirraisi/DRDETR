...
def box_convex_hull(box1, box2):
    ...
    return 0.5 * sum(
        p[0] * q[1] - p[1] * q[0]
        for p, q in zip(convex_hull, convex_hull[1:] + convex_hull[:1])
    )
    ...
    def intersection(self, others):
        ...
        inter = torch.stack(
            [
                ...
                (self.c * others.a - self.a * others.c) / w,
            ],
            dim=-1,
        )
        return inter  # [N, 2]
...
def cuts(polygons, sizes, p, q):
    ...
    v_abc = torch.stack(
        [lines(polygons_abc[:, k, :]) for k in range(K)], dim=1
    )  # [N, K]
    v_bca = torch.stack(
        [lines(polygons_bca[:, k, :]) for k in range(K)], dim=1
    )  # [N, K]

    # use new_polygons as stack and new_sizes as stack pointer
    # iterate and push new points
    new_polygons = (
        torch.zeros(N, K + 1, 2).to(polygons.device).fill_(1e5)
    )  # [N, K + 1, 2]
...
def box_inter_tensor(boxes1, boxes2):
    ...
    inter_xy = (
        boxes1.unsqueeze(1).expand(-1, M, -1, -1).reshape([N * M, 4, 2])
    )  # [N * M, 4, 2]
    cut_rect = (
        boxes2.unsqueeze(0).expand(N, -1, -1, -1).reshape([N * M, 4, 2])
    )  # [N * M, 4, 2]
    sizes = torch.zeros([N * M,]).to(boxes1.device).long().fill_(4)  # [N, M]

    inter_xy, sizes = cuts(
        inter_xy, sizes, cut_rect[:, 0, :], cut_rect[:, 1, :]
    )  # [N * M, 5, 2]
    inter_xy, sizes = cuts(
        inter_xy, sizes, cut_rect[:, 1, :], cut_rect[:, 2, :]
    )  # [N * M, 6, 2]
    inter_xy, sizes = cuts(
        inter_xy, sizes, cut_rect[:, 2, :], cut_rect[:, 3, :]
    )  # [N * M, 7, 2]
    inter_xy, sizes = cuts(
        inter_xy, sizes, cut_rect[:, 3, :], cut_rect[:, 0, :]
    )  # [N * M, 8, 2]
...
def box_convex_hull_tensor(boxes1, boxes2):
    ...
    boxes1 = (
        boxes1.reshape([-1, 4, 2])
        .unsqueeze(1)
        .expand(-1, M, -1, -1)
        .reshape([N * M, 4, 2])
    )  # [N * M, 4, 2]
    boxes2 = (
        boxes2.reshape([-1, 4, 2])
        .unsqueeze(0)
        .expand(N, -1, -1, -1)
        .reshape([N * M, 4, 2])
    )  # [N * M, 4, 2]
